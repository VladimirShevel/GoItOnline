...
public class Main {
    public static void main(String[] args) {
        ...
        ArrayList<String> encryptedCollection = encrypt(allInstruments);
        System.out.print("All instruments (encrypted) after your order \n");
        for (String instrument : encryptedCollection) {
            System.out.println(instrument);}

        ArrayList<String> decryptedCollection = decrypt(encryptedCollection);
        System.out.println("\n All instrumerts (decrypted) \n");
        for (String instrument : decryptedCollection) {
            System.out.println(instrument);}
    }
    //метод печати
    ...
    //метод шиврования коллекции
    public static ArrayList<String> encrypt(Collection<Instrument> inputCollection) {
        ArrayList<String> encryptedList = new ArrayList<String>();

        for (Instrument list : inputCollection) {
            encryptedList.add(Ceazar.encrypt(2, list.toString()));
        }
    return encryptedList;}
    //метод дешифровки коллекции
    public static ArrayList<String> decrypt(ArrayList<String> inputCollection) {
        ArrayList<String> decryptedList = new ArrayList<String>();
        for (String list : inputCollection) {
            decryptedList.add(Ceazar.decrypt(2, list));
        }
